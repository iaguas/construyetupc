...
function vShowComponentSelection($part) {
    ...
    switch($part){
        case 'cpu':
            ...
            // TODO: Rellenar listado de opciones
            // Se trocea la página, se rellenan las opciones con sus campos, y se vuelven a juntar las partes
            $pieces = explode("##listado##",$page);
            $aux = $pieces[1];
            $pieces[1] = "";
            foreach ($processors as $listItem){
                foreach ($listItem as $itemProp) {
                    $option = $aux;
                    $option = str_replace("##valor##",$itemProp[1],$option);
                    $option = str_replace("##nomcpu##",$itemProp[2],$option);
                    $pieces[1] = $pieces[1] . $option;
                }
            }
            $page = implode("",$pieces);
            // TODO: Filtrado de procesadores
            break;
        case 'cpu-cooler':
 ...
 * Muestra la lista de selección de modelo de componente
 */
function vShowComponentSearch($part,$id){
    //$part = $_GET['part']; // TODO: Eliminar esta línea cuando estemos seguros de que el AltoRouter funciona bien.
    switch($part){
        case 'cpu':
            $dhtml = '';
            ...
            // Mostrar el procesador coincidente con el nombre buscado
            $id = intval($id);
            $partItem = $processors[$id];
            $dhtml .= "<tr>";
